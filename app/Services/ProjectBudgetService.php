...
class ProjectBudgetService
{
    ...
    public function createBudget(array $data, array $items = [])
    {
        return DB::transaction(function () use ($data, $items) {
            ...
            if (isset($data['attachments']) && is_array($data['attachments'])) {
                $attachmentsPaths = [];
                foreach ($data['attachments'] as $category => $file) {
                    if ($file instanceof \Illuminate\Http\UploadedFile) {
                        $path = $file->store('budgets', 'public');
                        $attachmentsPaths[$category] = $path;
                    }
                }
    ...
    public function updateBudget(int $id, array $data, array $items = [])
    {
        ...
        return DB::transaction(function () use ($budget, $data, $items) {
            ...
            if (isset($data['attachments']) && is_array($data['attachments'])) {
                $attachmentsPaths = $budget->attachments ?? [];
                foreach ($data['attachments'] as $category => $file) {
                    if ($file instanceof \Illuminate\Http\UploadedFile) {
                        if (isset($attachmentsPaths[$category])) {
                            \Illuminate\Support\Facades\Storage::disk('public')->delete($attachmentsPaths[$category]);
                        }
                        $path = $file->store('budgets', 'public');
                        $attachmentsPaths[$category] = $path;
                    }
