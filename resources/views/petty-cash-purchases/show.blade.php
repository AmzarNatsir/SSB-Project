...
@section('title', 'Detail Pembelian Tunai')
@section('content')
@php $b = $purchase; @endphp
<div class="page-wrapper">
    <div class="content">
        <div class="d-md-flex d-block align-items-center justify-content-between mb-4">
            <div class="my-auto mb-2">
                <h3 class="page-title mb-1">
                    {{ $b->purchase_number }}
                    <span class="badge bg-{{ $b->status->color() }}-subtle text-{{ $b->status->color() }} fs-13 text-uppercase ms-2">{{ $b->status->label() }}</span>
                </h3>
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="{{ route('petty-cash-purchases.index') }}">Pembelian Tunai</a></li>
                        <li class="breadcrumb-item active">{{ $b->purchase_number }}</li>
                    </ol>
                </nav>
            </div>
            <div class="d-flex gap-2 align-items-center flex-wrap">
                @if($b->canEdit())
                    <a href="{{ route('petty-cash-purchases.edit', $b->uid) }}" class="btn btn-outline-primary btn-sm"><i class="ti ti-edit me-1"></i>Edit</a>
                @endif
                @if($b->canSubmit())
                    @if($hasApprovalMatrix)
                        <form action="{{ route('petty-cash-purchases.submit', $b->uid) }}" method="POST" class="d-inline js-confirm-form"
                              data-title="Ajukan Approval?" data-text="Pembelian akan dikirim ke {{ $nextApproverLabel }}."
        ...
        @if($b->canSubmit() && ! $hasApprovalMatrix)
            <div class="alert alert-warning border-warning">
                <h6 class="alert-heading"><i class="ti ti-alert-triangle me-1"></i>Matriks Approval Belum Diatur</h6>
                <p class="mb-2">Konfigurasi level approver untuk <code>PettyCashPurchase</code> belum dibuat.</p>
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header"><h5 class="card-title mb-0">Data Pembelian</h5></div>
                    <div class="card-body">
                        <dl class="row mb-0">
                            <dt class="col-sm-3 text-muted">Sumber Permintaan</dt>
                            <dd class="col-sm-9"><a href="{{ route('petty-cash-requests.show', $b->request->uid ?? '') }}" class="link-primary">{{ $b->request->request_number ?? '-' }}</a></dd>

                            <dt class="col-sm-3 text-muted">Proyek</dt>
                            <dd class="col-sm-9">{{ $b->project->project_name ?? '-' }} <small class="text-muted">({{ $b->project->project_code ?? '-' }})</small></dd>

                            <dt class="col-sm-3 text-muted">Tanggal Pembelian</dt>
                            <dd class="col-sm-9">{{ $b->purchase_date?->format('d M Y') }}</dd>

                            <dt class="col-sm-3 text-muted">Nomor PO</dt>
                            <dd class="col-sm-9">{{ $b->purchase_order_number ?? '—' }} <small class="text-muted">(modul Warehouse menyusul)</small></dd>

                            <dt class="col-sm-3 text-muted">Jenis Biaya</dt>
                            <dd class="col-sm-9">
                                @if($b->expenseCategory)
                                    <span class="badge bg-light text-dark">{{ $b->expenseCategory->code }}</span> {{ $b->expenseCategory->name }}
                                @else
                            ...
                            <dd class="col-sm-9">{{ $b->description }}</dd>

                            <dt class="col-sm-3 text-muted">Nominal Pembelian</dt>
                            <dd class="col-sm-9 fw-semibold fs-18 text-primary">Rp {{ number_format($b->amount, 0, ',', '.') }}</dd>

                            <dt class="col-sm-3 text-muted">Dibuat oleh</dt>
                            <dd class="col-sm-9">{{ $b->creator->name ?? '-' }} <small class="text-muted">({{ $b->created_at->format('d M Y H:i') }})</small></dd>

                            @if($b->approved_by)
                                <dt class="col-sm-3 text-muted">Disetujui oleh</dt>
                                <dd class="col-sm-9">{{ $b->approver->name ?? '-' }} <small class="text-muted">({{ $b->approved_at?->format('d M Y H:i') }})</small></dd>
                            @endif

                            <dt class="col-sm-3 text-muted">Lampiran</dt>
                            <dd class="col-sm-9">
                                @if($b->attachment_path)
                                    <a href="{{ route('petty-cash-purchases.attachment', $b->uid) }}" class="btn btn-soft-primary btn-sm"><i class="ti ti-download me-1"></i>Unduh</a>
                                @else
            ...
            <div class="col-lg-4">
                @if($b->canApprove() && $isCurrentApprover)
                    <div class="card border-warning">
                        <div class="card-header bg-warning-subtle"><h5 class="card-title mb-0 text-warning"><i class="ti ti-checks me-1"></i>Aksi Approval (Level {{ $b->current_approval_level }} dari {{ $flowLevels->count() }})</h5></div>
                        <div class="card-body">
                            ...
                            <form action="{{ route('petty-cash-purchases.approve', $b->uid) }}" method="POST" id="approve-form">
                ...
                @elseif($b->canApprove() && ! $isCurrentApprover)
                    <div class="card border-info">
                        ...
                        <div class="card-body">
                            <p class="mb-2">Pembelian menunggu persetujuan dari:</p>
...
(function() {
    ...
    $('button[data-approve-decision]').on('click', function() {
        ...
        Swal.fire({
            title: isReject ? 'Tolak Pembelian?' : 'Setujui Pembelian?',
            text: isReject ? 'Pastikan catatan penolakan diisi.' : 'Pembelian akan diteruskan ke level berikutnya (jika ada).',
