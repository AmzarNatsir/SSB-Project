...
@section('title', 'Detail SK Penugasan Tim')
@section('content')
<div class="page-wrapper">
    <div class="content">
        <div class="d-md-flex d-block align-items-center justify-content-between mb-4">
            ...
            <div class="d-flex gap-2 align-items-center flex-wrap">
                @if($formation->canEdit())
                    <a href="{{ route('workforce-formations.edit', $formation->uid) }}" class="btn btn-outline-primary btn-sm">
                ...
                @if($formation->canSubmit())
                    @if($hasApprovalMatrix)
                        <form action="{{ route('workforce-formations.submit', $formation->uid) }}" method="POST" class="d-inline js-confirm-form"
                              data-title="Ajukan untuk Approval?"
                              data-text="SK ini akan dikirim ke {{ $nextApproverLabel }} untuk diproses. Pastikan data anggota tim sudah benar."
                              data-icon="question"
                    ...
                    @else
                        <button class="btn btn-primary btn-sm" disabled
                                title="Matriks approval untuk SK Penugasan Tim belum diatur. Admin harus konfigurasi terlebih dahulu di Approval Matrix.">
                            <i class="ti ti-send me-1"></i> Ajukan Approval
                ...
                @if($formation->canActivate())
                    <form action="{{ route('workforce-formations.activate', $formation->uid) }}" method="POST" class="d-inline js-confirm-form"
                          data-title="Aktifkan SK?"
                          data-text="SK ini akan diaktifkan dan tim resmi mulai bertugas terhitung dari tanggal Berlaku Mulai."
                          data-icon="success"
                ...
                @if($formation->canRevise())
                    <form action="{{ route('workforce-formations.revise', $formation->uid) }}" method="POST" class="d-inline js-confirm-form"
                          data-title="Buat Revisi SK?"
                          data-text="SK aktif saat ini tetap berlaku sampai revisi baru disetujui. Versi baru akan dibuat dengan status Direvisi."
                          data-icon="warning"
                          data-confirm-text="Ya, Buat Revisi"
                ...
                @if($formation->canEnd())
                    <form action="{{ route('workforce-formations.end', $formation->uid) }}" method="POST" class="d-inline js-confirm-form"
                          data-title="Akhiri SK?"
                          data-text="Tindakan ini tidak dapat dibatalkan. SK akan berstatus Berakhir dan tidak bisa diaktifkan kembali."
                          data-icon="warning"
                          data-confirm-text="Ya, Akhiri"
        ...
        {{-- Banner: peringatan kalau matriks approval belum diatur (mencegah submit) --}}
                ...
                <div class="d-flex">
                    ...
                    <div class="ms-3">
                        ...
                        <p class="mb-2">
                            ...
                            untuk modul <code>SK Penugasan Tim</code>. Admin perlu mengatur minimal 1 level approver terlebih dahulu.
                        </p>
        ...
        {{-- Banner: info preview approver next level (sebelum disubmit) --}}
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                    ...
                    <div class="card-body">
                        <dl class="row mb-0">
                            ...
                            @if($formation->attachment_path)
                                ...
                                <dd class="col-sm-9">
                                    <a href="{{ route('workforce-formations.attachment', $formation->uid) }}" class="link-primary">
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0">Anggota Tim ({{ $formation->members->count() }})</h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-nowrap align-middle">
                                ...
                                <tbody>
                                    @forelse($formation->members as $i => $m)
                                        <tr>
                                            <td>{{ $i + 1 }}</td>
                                            <td>{{ $m->employee_name }}</td>
                                            <td>{{ $m->position_name }}</td>
                                            <td><span class="badge bg-light text-dark">{{ $m->shift }}</span></td>
                                    ...
                                    @empty
                                        <tr><td colspan="8" class="text-center text-muted py-4">Belum ada anggota tim.</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
            ...
            {{-- Right column --}}
            <div class="col-lg-4">
                {{-- Approval Action — hanya muncul kalau user adalah approver level ini --}}
                @if($formation->canApprove() && $isCurrentApprover)
                    <div class="card border-warning">
                        ...
                        <div class="card-body">
                            ...
                            @endif
                            <form action="{{ route('workforce-formations.approve', $formation->uid) }}" method="POST" id="approve-form">
                                @csrf
                ...
                {{-- Kalau SK dalam status SUBMITTED tapi user bukan approver — tampilkan info read-only --}}
                    ...
                    <div class="card border-info">
                        ...
                        <div class="card-body">
                            ...
                            <div class="alert alert-light border mt-3 mb-0 small text-muted">
                                ...
                                Anda tidak memiliki kewenangan untuk approval di level ini. Konfigurasi approver dapat diubah admin di
                                <a href="{{ route('approval-flows.index') }}" class="link-primary">Approval Matrix</a>.
                            </div>
                        </div>
                    </div>
                @endif
                            ...
                            <div class="d-flex mb-3">
                                ...
                                <div class="ms-3">
                                    <h6 class="mb-1">Level {{ $appr->level }}
                                        <span class="badge bg-{{ $statusColor }}-subtle text-{{ $statusColor }} ms-1">
                                            {{ $statusLabel }}
                                        </span>
                                    </h6>
...
<script>
(function() {
    // Intercept submit pada form .js-confirm-form — tampilkan SweetAlert dulu.
    // Submit hanya diproses kalau user klik konfirmasi.
    $(document).on('submit', '.js-confirm-form', function(e) {
        const $form = $(this);

        // Kalau sudah dikonfirmasi (flag di-set di submit ulang), lanjut submit beneran.
        ...
        Swal.fire({
            ...
            text:  $form.data('text')  || 'Lanjutkan tindakan ini?',
            icon:  $form.data('icon')  || 'question',
            showCancelButton: true,
            confirmButtonText: $form.data('confirm-text') || 'Ya, Lanjut',
    ...
    // Konfirmasi approve/reject di sidebar — pakai SweetAlert juga untuk konsistensi.
    // Beda dari form lain karena perlu set decision value dulu sebelum konfirmasi.
    $('button[data-approve-decision]').on('click', function() {
        ...
        Swal.fire({
            ...
            text:  isReject
                ? 'Pastikan catatan penolakan diisi supaya pengaju tahu apa yang perlu diperbaiki.'
                : 'SK akan diteruskan ke level berikutnya (jika ada).',
            ...
            inputValidator: isReject
                ? (value) => !value && 'Catatan penolakan wajib diisi.'
                : undefined,
        }).then(function(result) {
            if (! result.isConfirmed) return;

            $form.find('#approve-decision').val(decision);
            if (isReject && result.value) {
                $remarks.val(result.value);
            }
            // Raw DOM submit — bypass jQuery submit handlers
            $form[0].submit();
        });
    });
})();
