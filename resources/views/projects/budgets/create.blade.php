    ...
    <div class="page-wrapper">
        <div class="content">
            ...
            <form action="{{ route('budgets.store') }}" method="POST" id="create-budget-form" class="ajax-form">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select name="project_id" id="project_id" class="form-control" required>
                                        ...
                                        @if(isset($project))
                                            <option value="{{ $project->id }}" selected>{{ $project->project_name }}
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="profit_margin_percent" id="profit_margin_percent"
                ...
                <div class="card {{ !isset($project) ? 'd-none' : '' }}" id="project-info-card">
                    ...
                    <div class="card-body py-3">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <h6 class="fw-bold mb-0" id="info-project-code">{{ isset($project) ? $project->project_number : '-' }}</h6>
                            </div>
                            <div class="col-md-4">
                                <p class="text-muted mb-1 fs-13">Project Name</p>
                                <h6 class="fw-bold mb-0" id="info-project-name">{{ isset($project) ? $project->project_name : '-' }}</h6>
                            </div>
                            <div class="col-md-4">
                                <p class="text-muted mb-1 fs-13">Project Value</p>
                                <h6 class="fw-bold mb-0 text-primary" id="info-project-value">{{ isset($project) ? 'Rp ' . number_format($project->project_value, 0, ',', '.') : '-' }}</h6>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <ul class="nav nav-pills nav-fill mb-3 p-2 bg-light rounded" role="tablist">
                            @foreach(App\Enums\BudgetCategory::cases() as $category)
                                <li class="nav-item" role="presentation">
                                    <a class="nav-link {{ $loop->first ? 'active' : '' }} d-flex align-items-center justify-content-center gap-2"
                                        href="#tab_{{ $category->value }}" data-bs-toggle="pill" role="tab">
                        ...
                        <div class="tab-content pt-3">
                            @foreach(App\Enums\BudgetCategory::cases() as $index => $category)
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                    id="tab_{{ $category->value }}" role="tabpanel">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div class="col-md-5">
                                            <label class="form-label form-label-sm mb-1 text-muted"
                                                style="font-size: 0.8rem;">Upload Document (PDF, Word, Excel) - Optional</label>
                                            <input type="file" class="form-control form-control-sm file-upload-input"
                                                id="file_{{ $category->value }}" data-category="{{ $category->value }}"
                                        ...
                                        <table class="table table-bordered item-table" id="table_{{ $category->value }}">
                                            ...
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <td class="text-end fw-bold category-total"
                                                        data-category="{{ $category->value }}">Rp 0</td>
                                                    <td></td>
                                                </tr>
                                            </tfoot>
                                        </table>
            ...
            $(document).ready(function () {
                ...
                $('#project_id').on('select2:select', function (e) {
                    ...
                    if(data.project_number) {
                        $('#info-project-code').text(data.project_number);
                        $('#info-project-name').text(data.project_name);
                        $('#info-project-value').text('Rp ' + formatNumber(data.project_value || 0));
                        $('#project-info-card').removeClass('d-none');
                    }
                });

                $('#project_id').on('select2:unselect', function (e) {
                     $('#project-info-card').addClass('d-none');
                });
